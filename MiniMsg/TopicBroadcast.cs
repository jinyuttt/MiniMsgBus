...
namespace MiniMsg
{
    internal class TopicBroadcast
    {
        ...
        private void Ipc_ReceiveTopic(string arg1, byte[] arg2)
        {
            ...
            if(items.Item2.EndsWith(LocalNode.GUID))
            {
                return;//自己的不要
            }
            if (items.Item1 == "Global")
            {
             ...
             var msg = items.Item2.Substring(0, items.Item2.Length - LocalNode.GUID.Length);
        ...
        public List<string> PgmPub(string topic)
        {
            ...
            foreach (var p in lstNodeAddress)
            {
                try
                {
                    ...
                    ipc.ZmqIpcPub("minimsg", Encoding.UTF8.GetBytes(topic + "|" + p+LocalNode.GUID));
        ...
        private void Broadcast(bool ispgm)
        {
            ...
            foreach (var kv in dic)
            {
                foreach (var p in kv.Value)
                {
                    ...
                    {
                        ipc.ZmqIpcPub("minimsg", Encoding.UTF8.GetBytes(kv.Key + "|" + p + LocalNode.GUID));
                    }
